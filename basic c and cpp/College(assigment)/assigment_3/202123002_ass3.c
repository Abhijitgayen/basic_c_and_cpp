...
void test1(void){
    FILE *Filepoint;
    Filepoint = fopen("a3test1.txt", "r");
    if (Filepoint == NULL) {
    printf("\n\n\tError....I am not able to read this file\n\n");
    }
    else{
    	int DAta;
    	long long int TotalEle=1;
    	while (!feof(Filepoint)){
		 fscanf(Filepoint, "%d", &DAta);
          TotalEle++;
			 }
        int* arr1=(int*)malloc(TotalEle*sizeof(int));
        int* arr2=(int*)malloc(TotalEle*sizeof(int));
        rewind(Filepoint);
        long long int i=0;
        int keyELe;
         while (!feof(Filepoint)){
		 fscanf(Filepoint, "%d", &arr1[i]);
             i++;
			 }
			 i--;
...
void test3(void){
    FILE *Filepoint;
    Filepoint = fopen("a3challenge.txt", "r");
    if (Filepoint == NULL) {
    printf("\n\n\tError....I am not able to read this file\n\n");
    }
    else{
